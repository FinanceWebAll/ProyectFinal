...
function ProyeccionValor() {
  ...
  const calcularValorFuturo = () => {
    ...
    setResultado(res.toFixed(2)); // Establecer el resultado
  
    // Crear el objeto operacion después de calcular el resultado
    const operacion = {
      descripcion: 'Proyección de Valor Futuro y Presente',
      capital: principal,  // El valor del capital ingresado por el usuario
      tasa: tasa,          // Tasa de interés
      tiempo: tiempo,      // Tiempo en años
      monto: res.toFixed(2),    // El monto final calculado
      fecha: new Date().toLocaleString(),  // Fecha de la operación
    };
  
    // Obtener el historial actual de operaciones desde localStorage
